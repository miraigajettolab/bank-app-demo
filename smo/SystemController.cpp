...
void SystemController::doForwardStep()
{
    ...
    case GenerateTask:
        if (statistics->getTotalTasksCreated() <= totalTasksRequired) {
            int whereBlyat = bufferQueue->addTask(sourceDevices.at(deviceId)->generateNextTask(currentTime));
            if (whereBlyat == -1){
                eventSet.insert(SystemEvent(currentTime, EventType::TaskRejected));//gtfo
            } else {
                eventSet.insert(SystemEvent(currentTime, EventType::TaskBuffer, whereBlyat));//big brain
            }
            eventSet.insert(SystemEvent(currentTime + sourceDevices.at(deviceId)->nextTaskGenerationTime(),
                                        EventType::GenerateTask, deviceId));
            eventSet.insert(SystemEvent(currentTime, EventType::TaskUnbuffer, whereBlyat));//stuff happens here
    ...
    case TaskUnbuffer:
        if((*operatingDevices.begin())->isAvaliable()) {
            ...
            eventSet.insert(SystemEvent(currentTime, EventType::TaskStarted, currentDevice->getDeviceNumber())); //pls work
